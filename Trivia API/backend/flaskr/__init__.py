...
def create_app(test_config=None):
    ...
    def get_categories():
        categories = [Category.format(cat) for cat in Category.query.all()]
        result = {
            "success": True,
    ...
    def get_questions():
        ...
        categories = [Category.format(cat) for cat in Category.query.all()]
        questions_data = Question.query.paginate(
            page, QUESTIONS_PER_PAGE, False)
        questions = [Question.format(q) for q in questions_data.items]
        if len(questions) > 0:
            result = {
                "success": True,
                "questions": questions,
    ...
    def search_questions():
        if request.data:
            ...
            if request.args.get('page'):
                page = int(request.args.get('page'))
            question = request.get_json()['search_question']
            if question:
